...
def render_pdd(context=None, gantt_image_path: str | None = None):
    ...
    def _append_html(doc, html_str: str) -> None:
        ...
        emphasis, and clickable <a href>.
        """
        if not html_str:
            return

        # Normalize & ensure a block-level root so inline-only content isn't dropped
        s = (html_str or "").replace("&nbsp;", " ")
        if not re.search(r"</?(p|div|ul|ol|li|h\d|blockquote|pre|table|br)\b", s, flags=re.I):
            s = f"<p>{s}</p>"
        ...
        try:
            ...
            try:
                ...
                # installed but choked on this fragment; fall through
                pass
        except Exception:
    ...
    def _append_basic_html(doc, html_str: str) -> None:
        """
        Dependency-light fallback for common HTML generated by Quill:
        <p>, <div>, <ul>/<ol>/<li> (nested), <strong>/<em>/<u>/<s>, <code>, <pre>, <br>, <h1-3>, <blockquote>, <a>.
        Creates real bullets/numbers and clickable links.
        """
        try:
        ...
        # Helper: create a clickable hyperlink run
        def _add_hyperlink(paragraph, url: str, text: str):
            ...
            c = OxmlElement("w:color"); c.set(qn("w:val"), "0563C1"); rPr.append(c)  # default link blue
            new_run.append(rPr); t = OxmlElement("w:t"); t.text = text or url; new_run.append(t)
        ...
        def add_inline(p, node, bold=False, italic=False, underline=False, strike=False, mono=False):
            if isinstance(node, NavigableString):
                ...
                if t:
                    r = p.add_run(t)
                    r.bold = bool(bold); r.italic = bool(italic); r.underline = bool(underline)
            ...
            tag = node.name.lower()
            if tag in ("strong", "b"): nb = True
            elif tag in ("em", "i"):   ni = True
            elif tag == "u":           nu = True
            elif tag in ("s", "strike", "del"): ns = True
            elif tag == "code":        nm = True
            if tag == "br":
                p.add_run().add_break(); return
            if tag == "a":
                label = node.get_text("", strip=True); href = node.get("href") or ""
                _add_hyperlink(p, href, label or href); return
