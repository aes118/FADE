...
def render_pdd(context=None, gantt_image_path: str | None = None):
    ...
    def _append_html(doc, html_str: str) -> None:
        ...
        Prefer html2docx; if unavailable or it fails at runtime, fall back to a basic parser
        that supports paragraphs, lists (nested), headings, code/pre, blockquote, <br>, inline
        emphasis, hyperlinks, and tables.
        """
        if not html_str:
            return
        ...
        # Normalize common Quill underline spans to <u> so fallback can see them
        # e.g., <span style="text-decoration: underline;">X</span> -> <u>X</u>
        s = re.sub(
            r'<span\b[^>]*style="[^"]*text-decoration\s*:\s*underline[^"]*"[^>]*>(.*?)</span>',
            r"<u>\1</u>",
            s,
            flags=re.I | re.S,
        )

        # Ensure a block-level root so inline-only content isn't dropped
        if not re.search(r"</?(p|div|ul|ol|li|h\d|blockquote|pre|table|tr|td|th|br)\b", s, flags=re.I):
            s = f"<p>{s}</p>"

        # First choice: html2docx (if present and succeeds)
        try:
            from html2docx import HtmlToDocx
            parser = HtmlToDocx()
            try:
                parser.add_html_to_document(s, doc)
                return
            except Exception:
                pass
        except Exception:
        ...
        # Last resort: dependency-light parser
        _append_basic_html(doc, s)

    def _append_basic_html(doc, html_str: str) -> None:
        """
        Lightweight fallback for common HTML:
        <p>, <div>, <ul>/<ol>/<li> (nested), <strong>/<em>/<u>/<s>, <code>, <pre>, <br>,
        <h1-3>, <blockquote>, <a>, and <table>/<tr>/<td>/<th>.
        Creates proper bullets/numbers, clickable links, and basic tables.
        """
        try:
        ...
        # ---- Hyperlink helper (use Word "Hyperlink" style so it's blue+underlined) ----
        ...
        def add_inline(p, node, bold=False, italic=False, underline=False, strike=False, mono=False):
            ...
            if tag == "a":
                ...
                _add_hyperlink(p, href, label or href);
                return
            for ch in node.children:
                add_inline(p, ch, nb, ni, nu, ns, nm)
        ...
        # ---- Tables ----
        def render_table(tbl_tag: Tag):
            rows = tbl_tag.find_all("tr", recursive=False)
            if not rows:
                return
            # detect number of columns from first non-empty row
            first = next((r for r in rows if r.find_all(["td", "th"], recursive=False)), None)
            ncols = len(first.find_all(["td", "th"], recursive=False)) if first else 1
            tbl = doc.add_table(rows=0, cols=max(1, ncols))
            for tr in rows:
...
SUBCATEGORY_UNIT_SUGGESTIONS = {
    ...
    # Equipment & Technology
    "ICT Equipment (computers, phones, tablets)": ["per device"],
...
def render_project_detail():
    ...
    version = st.session_state.get("_resume_file_sig", "")  # set by importer
    PD["roles_table"] = st.data_editor(
        PD["roles_table"],
        num_rows="dynamic",
        use_container_width=True,
        key=f"pd_roles_editor__{version}",  # <-- key changes when a new file is imported
        column_config={
